...
void device_apps_list(const void *key, const void *value, void *context) {
  CFDictionaryRef app_dict = (CFDictionaryRef)value;
  assert(app_dict != NULL);

  CFStringRef app_key = (CFStringRef)key;
  assert(app_key != NULL);

  CFStringRef app_path = CFDictionaryGetValue(app_dict, CFSTR("Path"));
  assert(app_path != NULL);
  
  printf("[APPS] %s -> %s\n", CFStringGetCStringPtr(app_key, CFStringGetSystemEncoding()), CFStringGetCStringPtr(app_path, CFStringGetSystemEncoding()));
}

void apps_list(AMDeviceRef device) {
  CFDictionaryRef result;
  assert(AMDeviceLookupApplications(device, 0, &result) == 0);
  
  CFDictionaryApplyFunction(result, &device_apps_list, NULL);
  CFRelease(result);
}

CFStringRef copy_device_app_url(AMDeviceRef device, CFStringRef identifier) {
    CFDictionaryRef result;
    assert(AMDeviceLookupApplications(device, 0, &result) == 0);
    ...
    CFStringRef app_path = CFStringCreateCopy(NULL, CFDictionaryGetValue(app_dict, CFSTR("Path")));
    assert(app_path != NULL);
...
CFStringRef copy_disk_app_identifier(CFURLRef disk_app_url) {
    CFStringRef bundle_identifier = CFStringCreateWithFormat(NULL, NULL, CFSTR("%s"), app_path);
...
void write_gdb_prep_cmds(AMDeviceRef device, CFURLRef disk_app_url) {
    ...
    CFStringRef device_app_url_path = copy_device_app_url(device, bundle_identifier);
    CFURLRef device_app_url = CFURLCreateWithFileSystemPath(NULL, device_app_url_path, kCFURLPOSIXPathStyle, true);
...
void handle_device(AMDeviceRef device) {
    ...
    CFStringRef app_url = copy_device_app_url(device, CFStringCreateWithFormat(NULL, NULL, CFSTR("%s"), app_path));
    assert(app_url != NULL);
    printf("[****] App path: %s\n", CFStringGetCStringPtr(app_url, CFStringGetSystemEncoding()));
    // apps_list(device);
    CFURLRef url = CFURLCreateWithFileSystemPath(NULL, app_url, kCFURLPOSIXPathStyle, true);
...
int main(int argc, char *argv[]) {
    if (argc < 2 || argc > 4) {
        printf("usage: %s [-d] <app_id> [device_id]\n", argv[0]);
        exit(1);
    }
